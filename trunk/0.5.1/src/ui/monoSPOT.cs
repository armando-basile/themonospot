	...
	public class monoSPOT {
		...
		private void parseFile (string filename) 
		{
			...
			try 
			{
				...
				moviOffset = parser.m_MoviStart;
				moviSize = parser.m_MoviSize;
				moviFrames = parser.headerFile.dwTotalFrames;
				idxSize = parser.m_IdxSize;
				idxStart = parser.m_IdxStart;
				rec_ix = parser.rec_ix_presence;
			...
			informations.AppendValues("Packet Bitstream:", retInfos["Packet Bitstream:"].ToString());
		...
		{
			Console.WriteLine("callBackValues called");			
			SaveWindow= null;
		}
	}
