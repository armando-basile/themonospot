	...
	public class monoSPOTwait
	{
		...
		private byte[] processFrame(byte[] inByteArray, ref int frameLength)
		{
			...
			if (startPos >= 0)
			{
				...
				Array.Copy(tmpByteArray, startPos + _udToChange.Length , outByteArray, startPos + 12, tmpByteArray.Length - _udToChange.Length - startPos);
		...
		private int writeIdx1Chunk()
		{
			...
			while (tmpIdxPointer < _idxSize)
			{
				...
				tmpDWordArray = intToByteArray((int)framesOffset[frameCount]);
				for (int j=0; j<4; j++)
