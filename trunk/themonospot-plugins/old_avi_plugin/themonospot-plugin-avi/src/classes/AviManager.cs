...
namespace ThemonospotPluginAvi
{
	...
	public partial class AviManager
	{
		...
		private string userDataToChange = "";
		...
		/// <summary>
		/// Change FourCC of Codec
		/// </summary>
		/// <param name="aviStreamHeaderFourccValue">
		/// A <see cref="System.String"/>FourCC Type info in Header Stream (DESC)
		/// </param>
		/// <param name="videoStreamHeaderFourccValue">
		/// A <see cref="System.String"/>FourCC Compression info in Video Stream (NAME)
		/// </param>
        public void Change4CC(string aviStreamHeaderFourccValue, 
		                      string videoStreamHeaderFourccValue)
        {
			int videoTrack = -1;
			...
			for(int j=0; j<_aviContainer.AviStreams.Count; j++)
			{
				if (_aviContainer.AviStreams[j].AviStreamHeader.FccType == 
            ...
            byte[] tmpHeaderStreamArray = null;
			byte[] tmpVideoStreamArray = null;
						
			FileStream updaterSW = new FileStream(_aviFilePath , FileMode.Open ,FileAccess.ReadWrite);
			
			updaterSW.Seek(_aviContainer.AviStreams[videoTrack].Offset, SeekOrigin.Current);
			
			tmpHeaderStreamArray = utils.GetBytesFromFourcc(aviStreamHeaderFourccValue);
/*			Console.WriteLine(tmpASHarray[0].ToString("X2") + 
			                  tmpASHarray[1].ToString("X2") + 
			                  tmpASHarray[2].ToString("X2") + 
			                  tmpASHarray[3].ToString("X2"));
*/
			updaterSW.Write(tmpHeaderStreamArray, 0, 4);

			updaterSW.Seek(_aviContainer.AviStreams[videoTrack].AviVideoStreamHeader[0].Offset,
			               SeekOrigin.Begin);
			tmpVideoStreamArray = utils.GetBytesFromFourcc(videoStreamHeaderFourccValue);
...
*/
			updaterSW.Write(tmpVideoStreamArray, 0, 4);
