...
namespace ThemonospotPluginAvi
{
	...
	public class PluginFactory: MarshalByRefObject, IThemonospotPlugin
	{
		...
		private bool _isOperationChanged = false;
		
		public bool IsOperationChanged
		{
			get{return _isOperationChanged;}
			set{_isOperationChanged = value;}
		}
...
//			                       (double)(AverageVideoBitRate / vStream.DataRate));
		...
		/// <summary>
		/// Perform edit operation on file
		/// </summary>
		/// <param name="editOperation">
		/// A <see cref="System.String"/> Operation name
		/// </param>
		/// <param name="returnParameters">
		/// A <see cref="System.Object"/> Output parameters
		/// </param>
		/// <param name="editParameters">
		/// A <see cref="System.Object"/> Required parameters to perform operation
		/// </param>
		public void EditFile(string editOperation, 
		                 ref object[] returnParameters,
		       	      params object[] editParameters)
		{
			
			if (editOperation == "CHANGE4CC")
			{
				// Change FourCC
				string filePath = (string)editParameters[0];
				string newDesc = (string)editParameters[1];
				string newValue = (string)editParameters[2];
				OnOperationStatusChanged(this, new StatusChangedEventArgs("Changing FourCC values", 50));
				am = new AviManager(filePath, _debugMode);
				am.StatusChangedEvent += OnOperationStatusChanged;
				am.Change4CC(newDesc, newValue);
				OnOperationStatusChanged(this, new StatusChangedEventArgs("", 100));
				return;	
				
			}
			else if (editOperation == "USERDATA")
			{
				// Change UserData where there is DivX....
				string filePath = (string)editParameters[0];
				string outFilePath = (string)editParameters[1];
				am = new AviManager(filePath, _debugMode);
				am.StatusChangedEvent += OnOperationStatusChanged;
				am.ModifyUserData(outFilePath);
				return;
			}
		...
		/// </param>
		/// <param name="args">
		/// A <see cref="StatusChangedEventArgs"/>
		/// </param>
		public void OnOperationStatusChanged(object Sender, StatusChangedEventArgs args)
		{
			_operationDesc = args.StatusDescription;
			_operationState = args.StatusValue;
			_isOperationChanged = true;
		}
