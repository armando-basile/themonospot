...
namespace ThemonospotPluginAvi
{
	...
	public partial class AviManager
	{
		...
		private BytesManipulation byteMan = new BytesManipulation();
		private StreamManager streamMan = new StreamManager();
		
		// private bool recIX = false;
		private long streamPosition = 0;
		
		// Log4Net object
		private static readonly ILog log = LogManager.GetLogger(typeof(AviManager));
		...
		/// </param>
		public AviManager(string aviFilePath)
		{
		...
		public void GetInfo()
		{
			...
			FourCC = streamMan.ReadDWord(ref ifs, ref streamPosition);
			
			if ((byteMan.GetFourccFromInt(FourCC) != AviConstants.RIFF) &&
			    (byteMan.GetFourccFromInt(FourCC) != AviConstants.RIFX))
			{
				ifs.Close();
				ifs.Dispose();
				ifs = null;
				...
				throw new Exception("Error. Not a valid RIFF file\r\n" + 
				                    "[" + FourCC.ToString() + " " + 
				                          byteMan.GetFourccFromInt(FourCC) + 
				                    "]");
			}
			...
			uint dataSize = streamMan.ReadDWord(ref ifs, ref streamPosition);
			uint fileType = streamMan.ReadDWord(ref ifs, ref streamPosition);
			...
			// Debug
			log.Info(utils.GetRow("________", "FILE", 0, Convert.ToUInt32(_aviFileSize)));

			// Debug
			log.Info(utils.GetRow("________", "RIFF", 0, dataSize));
			log.Info(utils.GetRow(byteMan.GetFourccFromInt(fileType).PadRight(8, Convert.ToChar("_")) , 
			                      "TYPE", 0, 0));
			...
			while (streamPosition < _aviFileSize)
			{
				...
				FourCC = streamMan.ReadDWord(ref ifs, ref streamPosition);
				sFourCC = byteMan.GetFourccFromInt(FourCC);
				hFourCC = FourCC.ToString("X8");
				FourCClen = streamMan.ReadDWord(ref ifs, ref streamPosition);
				...
				// Debug
				log.Info(utils.GetRow(hFourCC,sFourCC, streamPosition, FourCClen));
				...
				{
					// recIX = true;
					log.Info(utils.GetRow(hFourCC, sFourCC, streamPosition, 0));
				}
				...
				{
					// unknow section... skip
					streamMan.StreamSeek(ref ifs, ref streamPosition, FourCClen);
				}
		...
		private void ManageLIST(uint sectionSize)
		{
			...
			uint FourCC = streamMan.ReadDWord(ref ifs, ref streamPosition);
			string sFourCC = byteMan.GetFourccFromInt(FourCC);
			...
			while (iPos < sectionSize)
			{
				...
				FourCC = streamMan.ReadDWord(ref ifs, ref streamPosition);
				uint FourCClen = streamMan.ReadDWord(ref ifs, ref streamPosition);
		...
		private void ManageIDX1(uint sectionSize)
		{
			...
			// Skip section
			streamMan.StreamSeek(ref ifs, ref streamPosition, sectionSize);			
		}
		...
		{
			// Read JUNK section
			byte[] tmpBytes = streamMan.StreamRead(ref ifs, ref streamPosition, sectionSize);

			string junkString = byteMan.GetAsciiFromBytes(tmpBytes).Trim();
		...
		private void ManageSTRF(uint sectionSize)
		{
			...
			{
				// "vids" section
				tmpBytes = streamMan.StreamRead(ref ifs, ref streamPosition, sectionSize);
		...
		private void ManageISFT(uint sectionSize, uint listLen)
		{
			...
			string isftString = byteMan.GetAsciiFromBytes(tmpBytes).Trim();
			...
			{
				streamMan.StreamSeek(ref ifs, ref streamPosition, (listLen - 8) - sectionSize);
			}
		...
		private void ManageMOVI(uint listLen)
		{
			...
			while (iPos < listLen)
			{
				    ...
				    sFourCC.Substring(2,2) == "db")
				{
					...
					if (frameCount >= 200)
	    			{
						streamMan.StreamSeek(ref ifs, ref streamPosition, (listLen - iPos));
						iPos = listLen;
	    			}
		...
		private void ParseDCuserdata(uint dcSubareaSize)
		{
			...
			byte[] dcBuffer = streamMan.StreamRead(ref ifs, ref streamPosition, dcSubareaSize);
			
			bool isAlreadyPresent = false;

			// Find UserData ... START			
			sPoint =  byteMan.CompareByteArray(dcBuffer, AviConstants.UserDataBytes, 0);
			
			// Loop to find END POINT
			while ((sPoint < (int)dcSubareaSize) && sPoint >= 0)
			{
				// Find another UserDataBytes
				ePoint = byteMan.CompareByteArray(dcBuffer, AviConstants.UserDataBytes, (sPoint + 3));
				
				if (ePoint < 0)
				{
					// Find VOLStartBytes
					ePoint = byteMan.CompareByteArray(dcBuffer, AviConstants.VOLStartBytes , (sPoint + 3));
				}
				
				if (ePoint < 0)
				{
					// Find VOPStartBytes
					ePoint = byteMan.CompareByteArray(dcBuffer, AviConstants.VOPStartBytes , (sPoint + 3));
				}
				
				if (ePoint < 0)
				{
					// from sPoint to end of Byte Array
					outValue = byteMan.GetHexFromBytes(dcBuffer, (sPoint+4), (int)(dcSubareaSize - (sPoint+3)) );
					outValue = byteMan.GetAsciiFromHex(outValue);
				...
				else
				{
					...
					outValue = byteMan.GetHexFromBytes(dcBuffer, (sPoint+4), (int)(ePoint - (sPoint+4)) );					
					outValue = byteMan.GetAsciiFromHex(outValue);					
					...
					sPoint = byteMan.CompareByteArray(dcBuffer, AviConstants.UserDataBytes, ePoint);
				}
			}
		...
		private void ParseDCvopdata(uint dcSubareaSize)
		{
			...
			sPoint =  byteMan.CompareByteArray(dcBuffer, AviConstants.VOPStartBytes, 0);
			
			while (sPoint < (dcSubareaSize-2) && sPoint >= 0)
			{
				vopCount++;
				sPoint =  byteMan.CompareByteArray(dcBuffer, AviConstants.VOPStartBytes, sPoint+3);
			}
